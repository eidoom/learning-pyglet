...
def player_lives(num_icons, batch=None):
    ...
    for i in range(num_icons):
        new_sprite = pyglet.sprite.Sprite(
            img=image,
            x=int(parameters.width - (i + 0.5) * width),
            y=int(parameters.height - height / 2 - parameters.margin),
            batch=batch,
        )
...
def asteroids(num_asteroids, player_position, batch=None):
    ...
    asteroid_block_radius = max(
        resources.asteroid_image.width, resources.asteroid_image.height
    )
    ...
    for i in range(num_asteroids):
        ...
        while util.distance(
            (asteroid_x, asteroid_y), player_position[:2]
        ) < player_block_radius or any(
            [
                util.distance((asteroid_x, asteroid_y), old_asteroid.position[:2])
                < asteroid_block_radius
                for old_asteroid in asteroids_list
            ]
        ):
            asteroid_x = random.randint(
                border_block_radius, parameters.width - border_block_radius
            )
            asteroid_y = random.randint(
                border_block_radius, parameters.height - border_block_radius
            )
        ...
        new_asteroid.velocity_x, new_asteroid.velocity_y = (
            random.random() * asteroid_max_velocity,
            random.random() * asteroid_max_velocity,
        )
